 ...
 * @returns
 */
export const getSession = (session_id: string): WASocket | undefined =>
  WaSessions.get(session_id)
...
export function onQRUpdated(
  listener: ({ session_id, qr }: IOnQRUpdated) => any
): void {
 ...
 * @returns
 */
export async function isExistWhatsapp(
  session_id: string,
  to: string,
  isGroup?: boolean
  ...
  try {
    ...
    const phone = phoneHelper.format(to)
    const receiver = phoneHelper.formatWhatsapp(String(phone))

    console.log({ phone, receiver })

    if (!isGroup) {
      const result = await session.onWhatsApp(receiver)
      const isExist = result?.[0]?.exists
      return Boolean(isExist)
    }

    const result = await session.groupMetadata(receiver)
 ...
 * @returns
 */
export function phoneToJid(phone: string, isGroup?: boolean): string {
  let number = phone

  number = phone.replace(/\s|[+]|[-]/gim, '')
